...
_plugin.register = function (plugin, options, next) {
	...
	languageLoader.setup({poDirectory: settings.poDirectory}, function (err, foundLocales) {
		...
		plugin.ext('onPreResponse', function (request, reply) {
			...
			// I can't think of any reason to worry about l8n when sending a file
			//if(response.variety && response.variety === 'file') return reply();

			var l8n = plugin.plugins["l8n-gettext"];

			if(response.headers)
				response.headers['content-language'] = l8n.locale.replace('_', '-'); //

			// new locale? Set the cookie.
			if (!request.state[settings.cookieName] || request.state[settings.cookieName] != l8n.locale)
				response.state(settings.cookieName, l8n.locale);

			// export l8n.* methods for selectedLocale to views
			if (response.variety === 'view') {
				response.source.context = response.source.context || {};
				response.source.context.l8n = l8n;
			}
	...
	function setLocale(locale, next) {
		...
		if(plugin.plugins["l8n-gettext"] && plugin.plugins["l8n-gettext"].locale == locale) return;

		plugin.log(['l8n-gettext', 'debug'], 'using locale: ' + locale);
