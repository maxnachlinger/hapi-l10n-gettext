...
/*
 ...
 -------------------------------
 POST - /_l10n with {_locale: 'en'} in the body
 GET - /_l10n/en
...
_plugin.register = function (plugin, options, next) {
	...
	languageLoader.setup(settings.l10nDirectory, function (err, foundLocales) {
		...
		if (!foundLocales || foundLocales.length === 0)
			return next(new Error('No loccales found in PO files'));
		...
		plugin.ext('onPreResponse', function (request, reply) {
			// see if we received a request for a new locale, if not try the cookie,
			// the header, then use the default locale for our locale.
			var requestedLocale = getNewLocale(request)
			...
			if(!response || response.isBoom) return reply();

			// new locale? Set the cookie.
			if (!request.state['_locale'] || request.state['_locale'] != requestedLocale)
				response.state('_locale', requestedLocale, {path: '/'});
	...
	function getNewLocale(request) {
		return (request.payload || {})['_locale']
			|| (request.params || {})['_locale']
			|| (request.query || {})['_locale'];
	}

	function setLocale(locale) {
		// locale hasn't changed
		var l10n = plugin.plugins['l10n-gettext'];
		if (l10n && l10n.locale == locale) return;

		plugin.log(['l10n-gettext', 'debug'], 'changed from locale: ' + (l10n || {}).locale + ' to ' + locale);
