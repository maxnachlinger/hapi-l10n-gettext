...
_plugin.defaults = {
	...
	ignorePath: '/public/', // ignore request's with a path containing this
...
_plugin.register = function (plugin, options, next) {
	...
	languageLoader.setup(settings.l10nDirectory, function (err, foundLocales) {
		...
		plugin.log(['hapi-l10n-gettext', 'debug'], 'done loading PO files, found locales: ' + util.inspect(foundLocales));
		var localeCodes = foundLocales.map(function(foundLocale) { return foundLocale.code; });
		...
		plugin.ext('onPreHandler', function (request, extNext) {
			if(!request.path.indexOf(settings.ignorePath)) return extNext();
