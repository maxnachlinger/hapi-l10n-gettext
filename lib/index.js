...
_plugin.defaults = {
	...
	includeL10nFunction: defaultIncludeL10nFunction // function(path) returning true if path needs l10n
...
var defaultIncludeL10nFunction = function(path) {
	return true;
};
...
_plugin.register = function (plugin, options, next) {
	...
	languageLoader.setup(settings.l10nDirectory, function (err, results) {
		...
		if(results.dev) {
			...
			plugin.log(['hapi-l10n-gettext', 'debug'], 'done loading PO files, found locales: ' + util.inspect(results.localeCodes));
		}
		...
		plugin.route({
			...
			config: {
				validate: {
					params: {
						_locale: Joi.string().allow(results.localeCodes).required()
					}
				}
			},
		...
		plugin.ext('onPreHandler', function (request, extNext) {
			...
			if(!settings.includeL10nFunction(request.path)) return extNext();

			// try the params, payload, cookie, the header, and then use the default locale for our locale.
