...
_plugin.register = function (plugin, options, next) {
	...
	if (settings.mode & modes.exclude) {
		...
		plugin.log(['hapi-l10n-gettext', 'debug'], 'excluding a few paths');
	}
	...
	if (settings.mode & modes.include) {
		...
		plugin.log(['hapi-l10n-gettext', 'debug'], 'including a few paths');
	}

	if (settings.mode & modes.none)
		plugin.log(['hapi-l10n-gettext', 'debug'], 'no path inclusions or exclusions');
	...
	languageLoader.setup(settings.l10nDirectory, function (err, results) {
		...
		plugin.ext('onPreHandler', function (request, extNext) {
			if (ignoreRequest(request)) {
				plugin.log(['hapi-l10n-gettext', 'debug'], request.path + ' ignored.');
				return extNext();
			}
			...
			plugin.log(['hapi-l10n-gettext', 'debug'], request.path + ' localized.');
			extNext();
		});
